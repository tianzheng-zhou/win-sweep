...
| **可疑服务扫描** | 12 项风险信号量化评分，措出残留/未签名/来路不明的服务 | `detect-suspicious.ps1` |
| **软件卸载与清理** | 扫描已安装软件并推荐卸载；用户通过设置/控制面板手动卸载后，扫描 6 类残留（服务、任务、启动项、目录、注册表、临时文件）并清除。winget/MSI 软件支持终端直接卸载，带超时保护 | `uninstall-software.ps1` |
...
- **强力卸载** — AI 诊断哪些该删，你在设置/控制面板手动卸载，然后 AI 扫描 6 类残留彻底清除。winget/MSI 软件可终端直接卸载（带超时保护）。强制删除只作为安全模式下的最后手段
...
管理员权限同时影响**诊断和修改**两个阶段。

**没有管理员权限时，诊断结果是不完整的** —— 服务二进制路径、HKLM 启动项、计划任务详情、签名检查等可能缺失或不准确。AI 会主动告诉你哪些信息缺失了。
...
> 不提权也能用——AI 会提前告知哪些结果不完整，由你决定是否继续。
...
| 操作 | 需要管理员 | 没有管理员时缺失什么 |
|------|------------|--------------------|
| 系统诊断 | **建议** | 服务二进制路径、启动账户、部分进程详情不完整 |
| 服务优化 | 是 | 会失败 |
| HKCU 启动项 | 否 | — |
| HKLM 启动项 | 是 | 无法读取或修改 |
| 计划任务清理 | 是 | 会失败 |
| 软件卸载 | 部分（HKCU 应用不需要，HKLM 应用需要） | 系统级应用无法卸载 |
| 软件残留清理 | 是 | 无法清理服务、HKLM 注册表、Program Files |
| 可疑服务检测 | 是 | 签名检查和服务详情不完整 |
| 变更验证 | 部分 | 部分检查会跳过 |
...
│   ├── uninstall-software.ps1  # 软件卸载与残留清理
...
└── references/                 # AI 参考文档（按需加载）
    ...
    ├── uninstall-guide.md      # 软件卸载判断框架与残留清理
