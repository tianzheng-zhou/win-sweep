...
| **软件卸载与清理** | 通过 winget 或原生卸载器卸载程序，然后扫描 6 类残留（服务、任务、启动项、目录、注册表、临时文件）并清除 | `uninstall-software.ps1` |
...
- **强力卸载** — 不只是跑卸载器：winget 优先策略，再扫描 6 类残留（孤儿服务、计划任务、启动项、目录、注册表、临时文件）彻底清除
